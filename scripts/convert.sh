...
#   claude-skills — Claude Agent Skills (~/.claude/skills/<slug>/SKILL.md) for one-pass prompts
...
#   hermes        — Hermes per-bot configs (bot.yaml + system.md) for deploying bots
...
# Trim a description to Claude Skills' 1024-char limit, cutting on a word
# boundary so the frontmatter stays valid.
cap_description() {
  local text="$1" max="${2:-1000}"
  if (( ${#text} <= max )); then printf '%s' "$text"; return; fi
  local cut="${text:0:max}"; cut="${cut% *}"
  printf '%s…' "$cut"
}

# Quote a value as a YAML single-quoted scalar (safe for colons, '#', etc.).
yaml_sq() { local s="${1//\'/\'\'}"; printf "'%s'" "$s"; }
...
convert_claude_skills() {
  local file="$1"
  local name description slug outdir outfile body

  name="$(get_field "name" "$file")"
  description="$(cap_description "$(get_field "description" "$file")")"
  slug="$(slugify "$name")"
  slug="${slug:0:64}"
  body="$(get_body "$file")"

  outdir="$OUT_DIR/claude-skills/$slug"
  outfile="$outdir/SKILL.md"
  mkdir -p "$outdir"

  # Claude Agent Skills: one SKILL.md per skill in ~/.claude/skills/<name>/.
  # The frontmatter name must match the dir; description (<=1024 chars) is what
  # Claude reads to decide when to invoke it. Framed here for single-pass use.
  cat > "$outfile" <<HEREDOC
---
name: ${slug}
description: $(yaml_sq "$description")
---
> **One-pass skill.** Apply the expertise below to the user's request in a single response — deliver the finished result; don't ask clarifying questions unless something essential is missing.
...
convert_hermes() {
  local file="$1"
  local name description emoji vibe slug outdir body
  ...
  # Hermes per-bot config: bot.yaml + a separate system.md (the persona / system
  # prompt), so the long body never has to be escaped into YAML. Deploy with
  # install.sh --tool hermes. Field names mirror common Hermes bot configs —
  # adjust to your Hermes version if they differ.
  cat > "$outdir/bot.yaml" <<HEREDOC
# Generated by scripts/convert.sh from the source agent — do not edit here;
# edit the agent .md and re-run convert. Installs to \$HERMES_BOTS_DIR
# (default ~/.hermes/bots/${slug}/).
bot:
  name: ${slug}
  display_name: $(yaml_sq "$name")
  description: $(yaml_sq "$description")
  enabled: true
  model: default                 # resolved by your Hermes model-routing
  system_prompt_path: ./system.md
  gateway:
    platforms: []                # e.g. [whatsapp, telegram, slack] — where this bot answers
  ...
  for dir in "${AGENT_DIRS[@]}"; do
    ...
    while IFS= read -r -d '' file; do
      ...
      case "$tool" in
        ...
        claude-skills) convert_claude_skills "$file" ;;
...
main() {
  ...
  local valid_tools=("antigravity" "claude-skills" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "kimi" "codex" "hermes" "all")
  local valid=false
  ...
  local tools_to_run=()
  if [[ "$tool" == "all" ]]; then
    tools_to_run=("antigravity" "claude-skills" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "kimi" "codex" "hermes")
  else
    tools_to_run=("$tool")
  fi
  ...
  if $use_parallel && [[ "$tool" == "all" ]]; then
    ...
    local parallel_tools=(antigravity claude-skills gemini-cli opencode cursor openclaw qwen codex hermes)
